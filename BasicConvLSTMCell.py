...
class BasicConvCTRNNCell(ConvRNNCell):
  """Basic Conv LSTM recurrent network cell. The
  """

  def __init__(self, shape, filter_size, num_features, time_const, forget_bias=1.0, input_size=None,
               state_is_tuple=False, activation=tf.nn.tanh):
    """Initialize the basic Conv LSTM cell.
    Args:
      shape: int tuple thats the height and width of the cell
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
      ...
      concat = _conv_linear([inputs, h], self.filter_size, self.num_features, True)

      # i = input_gate, j = new_input, f = forget_gate, o = output_gate
      #i, j, f, o = tf.split(3, 4, concat)
      j = concat

      new_c = c * (1-self._eta) + self._eta *j
...
class BasicConvCTNNCell(ConvRNNCell):
  """Basic Conv LSTM recurrent network cell. The
  """
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
      ...
      concat = _conv_linear([inputs], self.filter_size, self.num_features, True)
